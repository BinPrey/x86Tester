...
std::vector<ZydisMnemonic> generateTests(
...
static void printUsage()
{
    fmt::print("Usage: x86Tester-cli [options] [mnemonic ...]\n"
               ...
               "  --validate-only       Validate existing test data without generating new tests\n"
               "  --validate-full       Validates every case, default validation does only 3 tests per group\n"
               "  --skip-validation     Skip validation of generated test data (for speed)\n"
               "  --help                Show this help\n");
}
...
int main(int argc, char** argv)
{
    ...
    const auto nextArg = [&](int& i) -> std::string {
        if (i + 1 >= argc)
        {
            Logging::println("Missing value for {}", argv[i]);
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        else if (arg == "--threads" || arg == "-j")
        {
            ...
            {
                Logging::println("Invalid thread count: {}", val);
                return EXIT_FAILURE;
            }
        }
        else if (arg == "--isa")
    ...
    const auto filter = buildFilter(mnemonicNames, isaNames, categoryNames, excludeNames);
    if (!filter)
        return EXIT_FAILURE;
    ...
    const auto generated = validateOnly ? finalList : generateTests(finalList, mode, outputPath, force);

    if (!skipValidation)
        validateTests(finalList, mode, outputPath, validateFull);
